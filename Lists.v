...
Proof.
 intros p.
 destruct p as [m n]. simpl. reflexivity. Qed.
  (* FILL IN HERE *)
(** [] *)
...
Definition sum (b1 b2 :bag):bag := alternate b1 b2.
  (* FILL IN HERE *) 
...
(** Write down an interesting theorem about bags involving the
    ...
    stuck! *)
Theorem count_add : forall v :nat, forall s : bag ,
 count v (add v s)=S (count v s).
Proof.
 intros v s. simpl.
 assert (beq_nat v v =true) as H.
  induction v as [|v'].
  simpl. reflexivity.
...
Qed.
  ...
  (* FILL IN HERE *)
Theorem rec_snoc :forall l:natlist,forall n : nat ,
...
Proof.
 ...
 Case "l=n l'".
  simpl.
  rewrite->IHl'. rewrite->snoc_append. rewrite->snoc_append. 
...
Theorem cons_snoc_app:
 forall n:nat, forall l1 l2: natlist,
 n::(rev l1)++(rev l2)=rev(l2++(snoc l1 n)).
Proof. 
 intros n l1 l2.
 rewrite->distr_rev.
 assert (n::rev l1= rev (snoc l1 n)) as H.
  rewrite->rec_snoc. reflexivity.
...
SearchAbout sum.
Theorem count_sum: forall s : bag,
   count 0 (sum nil s)=count 0 nil + count 0 s.
Proof.
 intro s.
 induction s as [|v s'].
 Case "s=nil".
...
Theorem rev_rev : forall l : natlist,
  rev (rev  l) =l.
Proof.
 intro l.
 induction l as [|v l'].
...
Proof.
 intros l1 l2.
 intro H.
 assert(rev (rev l1) = rev (rev l2)) as Q.
  rewrite-> H. reflexivity.
  rewrite<- rev_rev. rewrite<-Q.
