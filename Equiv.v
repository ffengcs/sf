...
Proof.
  ...
  Case "->".
    intros.
    inversion H. subst. inversion H5. subst. apply H2.
  Case "<-". 
...
Proof.
  intros.
  split.
  Case "->". intros. inversion H0. subst.
          rewrite->H in H6. inversion H6.
          apply H7.
  Case "<-". intros. apply E_IfFalse. rewrite H. reflexivity.
          apply H0.
Qed.
  (* FILL IN HERE *)
(** [] *)
...
Proof.
  intros. split.
  Case "->". intros. inversion H. subst.
    SCase "beval st b = true". apply E_IfFalse.
       simpl. rewrite H5. reflexivity. apply H6.
    SCase "beval st b = false". subst. apply E_IfTrue. 
       simpl. rewrite H5. reflexivity. apply H6.
  Case "<-". intros. inversion H. subst.
    SCase "beval st (BNot b) = true".
       simpl in H5. apply E_IfFalse. apply negb_true_iff. apply H5.
       apply H6.
    SCase "beval st (BNot b) = false". subst.
       simpl in H5. apply E_IfTrue. apply negb_false_iff. apply H5.
       apply H6.
Qed.
  (* FILL IN HERE *) 
(** [] *)
...
(** Write an informal proof of [WHILE_false].
 ...
 _Theorem_: If [b] is equivalent to [BFalse], then [WHILE b DO c END]
            is equivalent to [SKIP].

   _Proof_: 

     - ([->]) We must show, for all [st] and [st'], that if [(WHILE b
        DO c END) / st || st'] then [c1 / st || st'].

       Proceed by cases on the rules that could possibly have been
      
       - Suppose the final rule rule in the derivation of [(WHILE b 
         DO c END) / st' || st'] was [E_WhileEnd].  We then have, by
         the premises of [E_Skip].

       - On the other hand, suppose the final rule in the derivation
         of [(WHILE b DO c END) / st'0 || st'] was [E_WhileLoop].
         beval st BFalse = true contra with bequiv b BFalse.

     - ([<-]) We must show, for all [st] and [st'], that if [SKIP / 
       st || st'] then [(WHILE b DO c END) / st || st'].

       Since [b] is equivalent to [BTrue], we know that E_WhileEnd
...
(** Explain what the lemma [WHILE_true_nonterm] means in English.
  If [b] is equivalent to [BTrue],then (WHILE b DO c END) will not stop.
(* FILL IN HERE *)
*)
(** [] *)
...
Proof.
  intros. split.
  Case "->".
   intros. apply ex_falso_quodlibet.
   generalize H0. apply WHILE_true_nonterm. apply H.
  Case "<-".
   intros. apply ex_falso_quodlibet. 
   generalize H0. apply WHILE_true_nonterm with (c:=SKIP).
   split.
Qed.
...
Proof.
  intros. split.
  Case "->".
   intros. unfold aequiv in H. replace (st') with (update st X (aeval st (AId X))).
   apply E_Ass. rewrite (H st). reflexivity. inversion H0. subst. simpl.
   apply functional_extensionality . intros. rewrite update_same. reflexivity. reflexivity.
  Case "<-".
   intros. unfold aequiv in H. inversion H0. subst.
   assert (st=update st X (aeval st e)) as HE. rewrite<- (H st). apply functional_extensionality.
   intros. simpl. rewrite update_same. reflexivity. reflexivity. replace (update st X (aeval st e)) with st.
   apply E_Skip.
Qed. 
  (* FILL IN HERE *) 
(** [] *)
...
Proof.
  ...
  Case "->". intros.
    ...
    induction H1;inversion HeqcSeq. subst. apply E_Seq with st'.
    apply H. apply H1_ . apply (H0 st'). apply H1_0.
  Case "<-". intros.
    remember (c1';; c2') as c'Seq.
    induction H1;inversion Heqc'Seq. subst. apply E_Seq with st'.
    apply H.  apply H1_ . apply (H0 st'). apply H1_0.
Qed.
...
Proof.
  unfold cequiv. unfold bequiv.
  intros. split.
  Case "->". intros.
    remember (IFB b THEN c1 ELSE c2 FI) as cIFB.
    induction H2;inversion HeqcIFB.
    SSCase "E_IfTrue". subst. apply E_IfTrue.
        rewrite<-H. apply H2. apply H0. apply H3.
    SSCase "E_IfFalse". subst. apply E_IfFalse.
        rewrite<-H. apply H2. apply H1. apply H3.
  Case "<-". intros.
    remember (IFB b' THEN c1' ELSE c2' FI) as c'IFB.
    induction H2;inversion Heqc'IFB.
    SSCase "E_IfTrue". subst. apply E_IfTrue.
        rewrite->H. apply H2. apply H0. apply H3.
    SSCase "E_IfFalse". subst. apply E_IfFalse.
        rewrite->H. apply H2. apply H1. apply H3.
...
Proof.
  ...
  Case "BLe".
    ...
    destruct a1'; destruct a2'; try reflexivity. (*SCase "a1->n1 a2->n2"*)
      simpl. destruct (ble_nat n n0).  reflexivity. reflexivity.
...
Proof. 
  ...
  Case "WHILE".
    assert (bequiv b (fold_constants_bexp b)).
      SCase "Pf of assertion". apply fold_constants_bexp_sound.
    destruct (fold_constants_bexp b) eqn:Heqb. 
    SCase "fold_constants_bexp b=Bture".
      apply WHILE_true. apply H.
    SCase "fold_constants_bexp b = BFalse".
      apply WHILE_false. apply H.
    SCase "fold_constants_bexp b = BEq a a0".
      apply CWhile_congruence. apply H. apply IHc.
    SCase "fold_constants_bexp b = BLe a a0".
      apply CWhile_congruence. apply H. apply IHc.
    SCase "fold_constants_bexp b = BNot b0".
      apply CWhile_congruence. apply H. apply IHc.
    SCase "fold_constants_bexp b = BAnd b0_1 b0_2".
      apply CWhile_congruence. apply H. apply IHc.
Qed.
    (* FILL IN HERE *) 
(** [] *)
...
Proof.
  ...
  induction a.
  Case "ANum n". simpl. reflexivity.
  Case "AId i". simpl. reflexivity.
  Case "APlus a1 a2". destruct a1.
    SCase "a1=ANum n". destruct n.
      SSCase "n=0". simpl. apply IHa2.
      SSCase "n=S n". simpl. rewrite<-IHa2. reflexivity.
    SCase "a1=AId i". simpl. rewrite IHa2. reflexivity.
    SCase "a1=APlus a1_1 a1_2". simpl. simpl in IHa1.
      rewrite<-IHa1. rewrite<-IHa2. reflexivity.
    SCase "a1=AMinus a1_1 a1_2". simpl. rewrite<-IHa2.
      simpl in IHa1. rewrite<-IHa1. reflexivity.
    SCase "a2=AMult a1_1 a1_2". simpl. rewrite<-IHa2.
      simpl in IHa1. rewrite<-IHa1. reflexivity.
  Case "AMinus a1 a2". simpl. rewrite<-IHa1. 
      rewrite<-IHa2. reflexivity.
  Case "AMult a1 a2". simpl. rewrite<-IHa1.
      rewrite<-IHa2. reflexivity.
Qed.
...
Proof.
  ...
  Case "BEq a a0". simpl.
    replace (aeval st a) with (aeval st (optimize_0plus_aexp a)).
    replace (aeval st a0) with (aeval st (optimize_0plus_aexp a0)).
    reflexivity. rewrite<-optimize_0plus_aexp_sound. reflexivity.
  ...
  Case "BLe a a0". simpl. 
    ...
    reflexivity. rewrite<-optimize_0plus_aexp_sound. reflexivity.
    rewrite<-optimize_0plus_aexp_sound. reflexivity.
  Case "BNot b". simpl. rewrite IHb. reflexivity.
  Case "BAnd b1 b2". simpl. rewrite IHb1. rewrite IHb2.
    reflexivity. Qed.
...
Proof. 
  ...
  induction c.
  Case "SKIP". simpl. apply refl_cequiv.
  Case "i ::= a".
    simpl. apply CAss_congruence. apply optimize_0plus_aexp_sound.
  Case "c1;; c2". apply CSeq_congruence. apply IHc1. apply IHc2.
  Case "IFB b THEN c1 ELSE c2 FI". simpl. apply CIf_congruence.
    apply optimize_0plus_bexp_sound. apply IHc1. apply IHc2.
  Case "WHILE b DO c END". simpl. apply CWhile_congruence.
    apply optimize_0plus_bexp_sound. apply IHc.
Qed.
Definition  optimize_fold_com  (c : com) : com :=
  optimize_0plus_com (fold_constants_com c).

Theorem optimizer_sound : 
   ctrans_sound  optimize_fold_com .
Proof.
  unfold ctrans_sound. unfold  optimize_fold_com . intros.
  apply trans_cequiv with (fold_constants_com c).
  apply fold_constants_com_sound.
  apply optimize_0plus_com_sound.
Qed.
Example optimize_fold_com_ex:
  optimize_fold_com 
    (* Original program: *)
    (X ::= APlus (ANum 0) (ANum 9);;
     Y ::= AMinus (AId X) (ANum 3);;
     IFB BEq (AMinus (AId X) (AId Y)) (APlus (ANum 2) (ANum 4)) THEN
       SKIP 
     ELSE
...
Proof.
  intros. induction H. 
  Case "ANum n". simpl. reflexivity.
  Case "AId Y". simpl. apply update_neq. apply H.
  Case "APlus a1 a2". simpl. rewrite->IHvar_not_used_in_aexp1.
    rewrite->IHvar_not_used_in_aexp2. reflexivity.
  Case "AMinus a1 a2". simpl. rewrite->IHvar_not_used_in_aexp1.
    rewrite->IHvar_not_used_in_aexp2. reflexivity.
  Case "AMult a1 a2". simpl. rewrite->IHvar_not_used_in_aexp1.
    rewrite->IHvar_not_used_in_aexp2. reflexivity.
Qed.
  (* FILL IN HERE *)
...
Definition subst_equiv_property_OK :=forall i1 i2 a1 a2,
  var_not_used_in_aexp i1 a1->
  cequiv (i1 ::= a1;; i2 ::= a2)
         (i1 ::= a1;; i2 ::= subst_aexp i1 a1 a2).
(*Theorem subst_equiv_property_OK  : forall i1 i2 a1 a2,
  var_not_used_in_aexp i1 a1->
  cequiv (i1 ::= a1;; i2 ::= a2)
         (i1 ::= a1;; i2 ::= subst_aexp i1 a1 a2).
Proof. *)
...
Proof.
  eapply E_Seq. apply E_Skip. apply E_HAVOC.
Qed.
(* FILL IN HERE *) 
(** [] *)
