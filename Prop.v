...
Proof.
  ...
  Case"m= S m'". simpl. apply b_sum with (m:=m'*n)(n:=n).
    apply B. apply IHm'.
Qed.
...
Proof.
  intros n B.
  apply g_plus3 with (n:=10+n).
  apply g_plus5 with (n:=5+n).
  apply g_plus5 with (n:=n).
  apply B.
Qed.
   (* FILL IN HERE *) 
(** [] *)
...
Proof.
   ...
   Case"g_plus3". apply g_plus3 with (n:=n+(3+n+0)). rewrite-> helper_g_times2.
                  apply g_plus3 with (n:=n+n+0). rewrite-> helper_g_times2 in IHgorgeous.
                  apply IHgorgeous.
   Case"g_plus5". apply g_plus5 with (n:=n+(5+n+0)). rewrite-> helper_g_times2. 
                  apply g_plus5 with (n:=n+n+0). rewrite-> helper_g_times2 in IHgorgeous.
                  apply IHgorgeous.
Qed.
   (* FILL IN HERE *) 
(** [] *)
...
Proof.
  intros n E.
  induction n as [|n'].
  Case "n=0". unfold even.  simpl. reflexivity.
  Abort.

(*不能在n上利用数学归纳法证明该定理，因为偶数n的前驱是n-2，不是n-1. FILL IN HERE *)
(** [] *)
...
Proof.
  ...
  Case"H1=ev 0". simpl. apply H2.
  Case"H1=ev(S(S n'))". simpl. apply ev_SS with (n:=n'+m). apply IHH1'.
Qed. 
  (* FILL IN HERE *) 
(** [] *)
...
Proof.
  ...
  Case "n=S n'". inversion E. simpl. apply H0.
Qed.
(* 同样可以证明，不过后面还是需要使用[inversion]  FILL IN HERE *)
(** [] *)
...
Proof. 
  intros n m H1 H2.
  induction H2.
  Case"E = ev_0". simpl in H1. apply H1.
  Case"E= ev_SS n'". simpl in H1. inversion H1. apply IHev. apply H0.
Qed.
...
Proof.
  intros.
  assert (ev (n+m+(n+p))) as P.
  apply ev_sum with (n := n + m)(m := n + p).
  apply H. apply H0. SearchAbout plus.
  rewrite<-plus_swap in P.
  rewrite->plus_assoc with (n:=n)(m:=n+m)(p:=p) in P.
  rewrite->plus_assoc with (n:=n)(m:=n)(p:=m) in P.
  rewrite<-plus_assoc with (n:=n+n)(m:=m)(p:=p) in P.
  apply ev_ev__ev  with (n:=n+n)(m:=(m+p)) in P. apply P.
...
Inductive pal (X:Type) : (list X ) ->Prop:=
  |p_nil : pal X nil
  |p_h : forall (h:X) , pal X  (cons h nil)
  |p_hlh : forall (h:X) (l:list X), pal X l-> pal X (snoc (h::l) h)
  |p_revl: forall (l : list X), l=rev l-> pal X l.
 
Theorem palindrome_l_revl : forall (X:Type) (l:list X),
 pal X ( l ++ rev l).
Proof.
 intros.
 induction l as [| h l'].
 Case "l=[]". simpl. apply p_nil.
 Case "l=h::l'".  simpl.
 rewrite<-snoc_with_append.
 apply p_hlh.
 apply IHl'.
Qed.
Theorem palindrome_l_rel_beq : forall (X:Type) (l:list X),
 pal X l -> l = rev l.
Proof.
  intros.
  induction H as [|h'| h' l'|l'].
  Case "p_nil". simpl. reflexivity.
  Case "p_h". simpl. reflexivity.
  Case "p_hlh". rewrite-> rev_snoc. simpl.
...
Inductive subseq:list nat-> list nat ->Prop:=
  |subseq_nil_l : forall (l:list nat),subseq nil l
  |subseq_h_l: forall (h: nat) (l :list nat) ,subseq [h] ([h]++l)
  |subseq_ll1_l1_ll2_l2: forall (ll1:list nat)(ll2:list nat)(l1:list nat)(l2:list nat), 
        subseq ll1 l1->subseq ll2 l2->subseq (ll1++ll2) (l1++l2)
  |subseq_tran: forall (l1:list nat)(l2 :list nat)(l3 :list nat),
...
Theorem subseq_sum:forall (l1:list nat)(l2:list nat)(l3:list nat),
  subseq l1 l2->subseq l1 (l2++l3).
Proof.
  intros. 
  rewrite<-app_nil_end' with (l:=l1). 
  apply subseq_ll1_l1_ll2_l2 with (ll1:=l1)(ll2:=nil)(l1:=l2)(l2:=l3).
  apply H.
  apply subseq_nil_l.
Qed.
Theorem subseq_transitive:forall (l1:list nat)(l2:list nat)(l3:list nat),
  subseq l1 l2->subseq l2 l3->subseq l1 l3.
Proof.
  intros.
  apply subseq_tran with (l2:=l2).
  apply H. apply H0.
Qed.
(* FILL IN HERE *)
(** [] *)
