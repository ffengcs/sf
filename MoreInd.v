...
(** natlist1:
  forall P : natlist -> Prop
     P nnil1 ->
     (forall l:natlist ,P l ->forall n:nat, P ncons l n)->
...
(** byntree:
      ...
      (forall (y:yesno)(b:byntree), P b->forall b0:byntree, P b0->P (nbranch y b b0)->
...
(** tree:
  forall (X:Type)(P:tree X->Prop),
   ...
   forall t:tree X, P t->forall t0:tree X,P (node X t t0)->
  forall t :tree X, P t 
 *)
     ...
     foo'_ind :
        forall (X : Type) (P : foo' X -> Prop),
              (forall (l : list X) (f : foo' X),
                    ...
                    ______P (C1 X l f)_________________   ) ->
             _____________P (C2 X)______________________________ ->
...
(** Rewrite both [plus_assoc'] and [plus_comm'] and their proofs in
    ...
    defined proposition.  *) 
Definition plus_assoc_Def:nat->nat->nat->Prop:=
  fun n m p=>n + (m + p) = (n + m) + p.
Definition plus_comm_Def:nat->nat->Prop:=
  fun n m=>n+m=m+n.
(* FILL IN HERE *)
(** [] *)
...
(** Suppose we make the following inductive definition:
   ...
   foo_ind
        : forall (X Y : Set) (P : foo X Y -> Prop),   
          (forall x : X, __P (foo1 X Y x)__________________) ->
          (forall y : Y, __P (foo2 X Y y)__________________) ->
          (__forall f:foo X Y, P f1->P(foo3 X Y f)_________) ->
           __forall_f1:foo X Y, P f1.______________________
...
(** Consider the following induction principle:
   ...
   Inductive bar : Set :=
     | bar1 : _forall n:nat,bar'______________________
     | bar2 : _bar'->bar'_____________________________
     | bar3 : _bool->bar'->bar'_______________________.
...
(** Given the following inductively defined proposition:
  ...
  no_longer_than_ind
       : forall (X : Set) (P : list X -> nat -> Prop),
         (forall n : nat, _P [] n___________________) ->
         (forall (x : X) (l : list X) (n : nat),
          no_longer_than X l n -> _____P l n_________ -> 
                                  _____P (x::l) (S n)____)____ ->
         (forall (l : list X) (n : nat),
          no_longer_than X l n -> _____P l n_________ -> 
                                  _____P_l (S n)_________)____ ->
         forall (l : list X) (n : nat), no_longer_than X l n -> 
           _____P l n_______________
