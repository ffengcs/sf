...
[destruct]:
 分类讨论，可以将像bool型的变量这样具有有限个值的变量进行分类讨论。
 也了可以将自然数变量n分为 0 和 S n’，但是前提是证明n=S n’情况下成立时 不要求知道n’的值
[induction]:
  数学归纳法，将自然数变量n分为0和S n’ ，在证明n=S n’情况时 并不要求知道n’的值而是假设n’情况成立
(* FILL IN HERE *)
...
Theorem negb_negb_n_n : forall n : bool ,
    negb (negb n)=n.
Proof.
 intro n.
 destruct n.
...
(** This exercise is a continuation of the previous exercise about
    ...
    here. 
*)
Fixpoint conv_nat_bin (n:nat):bin:=
 match n with
 |O => Q 
 |S n' => increment (conv_nat_bin n')
 end.

Eval compute in conv_bin_nat (conv_nat_bin(7)).
Eval compute in conv_nat_bin (conv_bin_nat(M(T(T Q)))).
(**只要证明nat->bin->nat相反方向无需证明，因为该证明了nat->bin->nat
就意味着证明了nat->bin与bin->nat都是正确的，所以bin->nat->bin方向
的转换也必然正确*)
Definition normalize(b:bin):bin:= conv_nat_bin (conv_bin_nat(b)).
Eval compute in normalize(T(M(T Q))).
(* FILL IN HERE *)
(** [] *)
...
(** - _Theorem_: For any [n], [m] ,
      n + m = m + n.
    _Proof_: By induction on [n].

    - First, suppose [n = 0].  We must show 
        0 + m = m + 0 . 
      next:plus_0_r:m + 0 = 0 + m
      This follows directly from the definition of [+].
...
(** Write an informal proof of the following theorem, using the
    ...
    Proof:By induction on [n].
         ...
         directly from the definition of [beq_nat].
             true = beq_nat n' n'
          which is immediate from the induction hypothesis.
[]
 *)
