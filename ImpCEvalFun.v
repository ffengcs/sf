...
Lemma l1 :forall i1 i2 :nat,
  i1<=i1+i2.
Proof.
  induction i1 as [|i1']. intros.
     apply  ble_nat_true. apply AExp.foo.
     intros.
     simpl. apply n_le_m__Sn_le_Sm.  apply (IHi1' i2).
Qed.
...
Proof. 
  ...
  assert (ceval_step st c1 (i1+i2) = Some st') as H. 
    apply ceval_step_more with (i1:=i1). apply l1. apply E1. rewrite->H.
    apply ceval_step_more with (i1:=i2). rewrite->plus_comm. apply l1. apply E2.
  (*Case: IFB b THEN c1 ELSE c2 FI : beval st b = true*)
  inversion IHHce as [i E]. exists (S i). simpl. rewrite->H. apply E.
  (*Case: IFB b THEN c1 ELSE c2 FI : beval st b = false*)
  inversion IHHce as [i E]. exists (S i). simpl. rewrite->H. apply E.
  (*Case: WHILE b DO c END : beval st b = false*)
  exists 1. simpl. rewrite->H. reflexivity.
  (*Case: WHILE b DO c END : beval st b = true*)
  inversion IHHce1 as [i1 E1].
  inversion IHHce2 as [i2 E2].
  exists (S (i1+i2)). simpl. rewrite->H.
  assert (ceval_step st c (i1 + i2)=Some st') as HE. 
    apply ceval_step_more with (i1:=i1). apply l1. apply E1. 
    rewrite->HE. apply ceval_step_more with (i1:=i2). rewrite->plus_comm.
    apply l1. apply E2.
Qed.
