...
Fixpoint split
 ...
 match l with
  |nil=>(nil,nil)
  |(h1,h2)::l'=>(h1::fst(split l'),h2::snd(split l')) 
 end.
...
Definition filter_even_gt7 (l : list nat) : list nat :=
 filter evenb (filter (blt_nat 7) l).
  (* FILL IN HERE *)
...
(** Use [filter] to write a Coq function [partition]:
  partition : forall X : Type,
   ...
   list.
*)
Definition negb_fun {X:Type}(test:X->bool)(x:X):bool:=
  match test x with
   |true=>false
   |false=>true
  end.
Definition partition {X : Type} (test : X -> bool) (l : list X)
                     : list X * list X :=(filter test l ,filter (negb_fun test) l).
...
Proof.
 ...
 Case "l=nil".
  simpl. reflexivity.
 Case "l=cons v l'".
  simpl. rewrite->map_snoc. rewrite<- IHl'.
  reflexivity.
Qed.
  (* FILL IN HERE *)
(** [] *)
...
(** Observe that the type of [fold] is parameterized by _two_ type
    ...
    different? *)
Example fold_example4: fold cons [1;2;3;4;5;6;7;8;9] nil=[1;2;3;4;5;6;7;8;9].
 simpl. reflexivity. Qed.
(*X=nat;Y=listnat*)
...
Example test_fold_length1 : fold_length [4;7;0;3] = 4.
Proof. reflexivity. Qed.
...
(** Write down a theorem in Coq stating that [fold_map] is correct,
    and prove it. *)
Theorem map_fold_map : forall l : list nat, 
  map plus3 l=fold_map plus3 l.
Proof.
 intros.
 induction l as [|n l'].
 Case "l=nil".
